          ...
          <li key={i} className="mb-2">
            <div className="flex items-center justify-center">
              <a href="#" aria-label="Tops">
                <img className="w-60 h-60 object-cover" src="/images/Tops.png" alt="Tops" />
                {item.name}
                <div>{item.size}</div>
                <div>${item.price}</div>
                <div onClick={() => handleFavoriteClick(item, i)} style={{ cursor: 'pointer', fontSize: '24px' }}>{item.favorite ? "❤️" : "♡"}</div>
              </a>
            </div>
          </li>
        ))}
